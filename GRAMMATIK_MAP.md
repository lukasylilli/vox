...
# Stand: 2026-09-18 · Stufen G1–G6 umgesetzt (alle 84 Kern-Lektionen live) · G7a: 336 Übungen live · G7b: Niveau-Test A1–C2 live · G7c: Übungen aus Beispielsätzen live
...
│      └─ /uebung                 [G7a/G7c ✅] GrammatikUebungScreen — Quell-Übungen + je Beispielsatz eine erzeugte, gemischt
│
...
| **G7c** | Übungen aus den Beispielsätzen der Lektionen (nur Formen, die ohne Raten richtig sind) | ✅ 2026-09-16 |
...
| 2026-09-16 | G7c | **Übungen aus den 504 Beispielsätzen live** (Lukas: „vielfältig und zufällig"). `models/beispiel_uebungen.dart`: **je Beispielsatz genau eine Übung**, Art zufällig unter denen, die für diesen Satz ohne Raten stimmen — *bedeutung* (Satz → 4 Bedeutungen), *satzWahl* (Bedeutung → 4 Sätze), *richtigFalsch* (passt die gezeigte Bedeutung?), *matching* (3 Sätze ↔ 3 Bedeutungen, rechte Seite nie in linker Reihenfolge), *wordOrder* (Satz legen, **erstes Wort vorgegeben**, Bedeutung als Hilfe; nur einfache Sätze: ein Satz, 4–10 Wörter, keine →/+/Klammern/Doppelpunkte/Striche/Zitate). Ablenker nur aus derselben Lektion und in DE, FA **und** EN vom richtigen verschieden; alle Texte stehen so in der Quelle, nichts wird erfunden. Eine Lektionsrunde = 4 Quell-Übungen + 6 erzeugte = **10, gemischt**; „دوباره از اول" erzeugt neu. Niveau-Test: nimmt die erzeugten dazu, **außer Satzbau** (eine zweite richtige Wortstellung ist auch mit festem Anfang nicht ganz ausgeschlossen — Regel `GrammatikUebung.testTauglich`). Modell: drei neue Arten (nur für erzeugte, `ausJson` lehnt sie ab), Felder für übersetzte Optionen, Vorgabe, Beispiel-Rückmeldung. Nach dem Prüfen zeigt jede erzeugte Übung den Lektionssatz mit Bedeutung (+ `note`, falls vorhanden). **Nicht gebaut:** Lückentext aus Beispielsätzen — welches Wort fehlt und welche Ablenker sicher falsch sind, lässt sich aus den Daten nicht zuverlässig bestimmen (z. B. `weil`/`da` beide richtig). Wächter: `test/grammatik_uebungen_test.dart` (Gruppe G7c + Bedienung aller erzeugten in EN/FA), `test/grammatik_niveautest_test.dart`. |
| 2026-09-18 | L.6 | **Buch-Lektionen von Lukas, Folge 1.** Lukas schickt die Grammatik-Lektionen seiner Bücher der Reihe nach; seine Nummerierung ist NICHT die der App — die Zuordnung macht Claude und hält sie in PLAN.md → L.6 fest. ⚠️ **Kein Satz aus dem Buch wird übernommen:** aus dem Buch kommt nur das Thema, alle Erklärungen, Beispiele, Tabellen und Übungen schreibt Claude neu (andere Nomen, andere Sätze). **Lektion 1 „Grammatik aktiv" (Personalpronomen) → App-Lektion `personalpronomen`** (pronomen.json): neu sind die Blöcke „Person, Zahl und Anrede" und „Pronomen für Sachen", die Tabellen „Nominativ: Person und Zahl" und „Welches Pronomen für welches Nomen?", 4 Beispiele (ex-7…ex-10) und 6 Übungen (ex-pers-5…ex-pers-10: 1 multipleChoice, 2 fillBlank, 1 matching, 1 wordOrder, 1 transform). Bestand danach: 342 Übungen (336 + 6); A1-Vorrat des Niveau-Tests 91 (+5, transform zählt dort nie). Nichts Vorhandenes wurde gelöscht oder umgeschrieben. Tests angepasst: `grammatik_uebungen_test.dart`, `grammatik_niveautest_test.dart`. |
