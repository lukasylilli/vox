...
#   + بندهای ۲–۴ «قبل از انتشار» (پاپ‌آپ شکل‌ها، RTL، تلگرام) ✅ تأیید Lukas ⇒ اپ آماده‌ی انتشار؛ فقط L.6 (اختیاری) باز.
...
# دور ۶۴ (2026-09-24): L.3c تلگرام ⇒ web/telegram_guard.js (قبل از flutter_bootstrap.js؛ فقط اگر آدرس tgWebAppData دارد: SDK تلگرام + پاک‌کردن آدرس)
...
- **پیشرفت (2026-09-26 دور ۲۴۳):** آخرین کلمه‌ی ساخته‌شده **gemeinsam** · بعدی **gemeinslawisch** · کنار گذاشته: abatisch، abdikativ، afrikaans، aldente، aleppinisch، allenfallsig، altaltbacken، Altdorfer، altkrank، amaranten، ambient، amethysten، amphibolisch، anamorph، anelliert، anotherm، antichretisch، appendikuliert، aretologisch، arschig، arschlos، assi، athermisch، ausheimisch، austral، averbal، azentrisch، azephal، bananig، bannig، basalten، basiklin، basten، batisten، bebuscht، bebust، bedonnert، befotzt، begeißelt، begrannt، behemdet، behost، behuft، beinfarben، bekrallt، bemähnt، bemützt، bepelzt، beredet، berindet، bernsteinen، berstig، berüscht، beschilft، beschürzt، besoffenbesondere، besonderes، besonderer، bestiefelt، bestrumpft، bestusst، betresst، betrieben، bezastert، bezopft، bibliophob، biereifrig، bimaxillär، birken، bitchig، blakig، bland، blaustrümpfig، bonfortionös، botrytisiert، boustrophedon، brennheiß، brillanten، brokaten، buchen، bundrein، buntfarben، busig، bustrophedon، chemometrisch، chilotisch، chionophil، chochem، chondritisch، claviform، damasten، dasig، debitorisch، dekremental، delatorisch، demanten، deprekativ، depretiativ، desiderat، designatus، desmal، dextral، dezisionistisch، diamanten، diffusibel، dikasterial، dilemmatisch، diptotisch، dissidentisch، dissipativ، distich، dolent، doloros، dominicanisch، doppelherzig، drahten، drittelzahlig، dutzend، dyadisch، ebengleich، egressiv، ehrenkäsig، eiben، eichen، einschaltbereit، elektrogen، elenktisch، emeritus، emisch، empathogen، enaktiv، endogenetisch، endothym، endozentrisch، enkaptisch، enneaeterisch، enostal، entaktogen، ephebisch، epifaszial، epithetisch، eponymisch، erdhaltig، erethisch، erfolgswirksam، erlen، erotomanisch، erzen، eschen، espen، etisch، euploid، europazentriert، euryhalin، euryhygr، euryök، eurytherm، exergon، exogenetisch، exozentrischg، expert، extinkt، extradiegetisch، extramundan، extra-temporal، extratemporal، fabisch، fadenbreit، fadenlos، faselnackend، fasennackend، fassettenartig، fassettenreich، fastidiös، fekund، fennoskandisch، fichten، fickerig، filamentartig، fipsig، firn، flächengroß، flächsen، flächsern، flanellen، fluvioglazial، gasig، gebalkt، gebuckelt، gefizt، gegengleich، gegiebelt، gehäusetragend، geheist، gehenkelt، gehl، geköpert، gekröpft (فهرست کامل: PLAN.md → «🏁 مرحله‌ی آخر») · آرشیو: ۲۰۲۶ کارت.
- **2026-09-26 (دور ۲۱۴–۲۲۳):** ۱۰۰ کارت صفت در `assets/vocab/adjektiv/` (feinkörnig … formidabel)؛ ۱۲ کلمه‌ی نامطمئن در `zurueckgestellt` (PLAN «🏁 مرحله‌ی آخر» #175–186). ایندکس‌ها تازه شدند. هیچ فایل کد/ظاهر عوض نشد.
- **2026-09-26 (دور ۲۰۴–۲۱۳):** ۱۰۰ کارت صفت در `assets/vocab/adjektiv/` (eventuell … feingliedrig)؛ ۱۷ کلمه‌ی نامطمئن در `zurueckgestellt` (PLAN «🏁 مرحله‌ی آخر» #158–174). ایندکس‌ها تازه شدند. هیچ فایل کد/ظاهر عوض نشد.
- **2026-09-26 (دور ۱۹۴–۲۰۳):** ۹۹ کارت صفت در `assets/vocab/adjektiv/` (entwaffnend … evasiv) + ۱ کارت `assets/vocab/numerale/numerale_erste.json` (`wortartKorrektur`)؛ ۱۹ کلمه‌ی نامطمئن در `zurueckgestellt` (PLAN «🏁 مرحله‌ی آخر» #139–157). ایندکس‌ها تازه شدند. هیچ فایل کد/ظاهر عوض نشد.
- **2026-09-26 (دور ۱۸۴–۱۹۳):** ۹۸ کارت صفت در `assets/vocab/adjektiv/` (einvernehmlich … enttäuscht) + ۲ کارت `assets/vocab/numerale/` (einundzwanzigste، elfte؛ `wortartKorrektur`)؛ ۱۳ کلمه‌ی نامطمئن در `zurueckgestellt` (PLAN «🏁 مرحله‌ی آخر» #126–138). ایندکس‌ها تازه شدند. هیچ فایل کد/ظاهر عوض نشد.
- **2026-09-26 (دور ۱۷۴–۱۸۳):** ۹۷ کارت صفت در `assets/vocab/adjektiv/` (dunstig … einträglich) + ۳ کارت `assets/vocab/numerale/` (einhunderterste، einhundertste، einunddreißigste)؛ `wortartKorrektur` برای این‌ها و einundzwanzigste؛ ۸ کلمه‌ی نامطمئن در `zurueckgestellt` (PLAN «🏁 مرحله‌ی آخر» #118–125). ایندکس‌ها تازه شدند. هیچ فایل کد/ظاهر عوض نشد.
- **2026-09-26 (دور ۱۶۴–۱۷۳):** ۹۵ کارت صفت در `assets/vocab/adjektiv/` (diskriminierend … dunkelrot) + ۵ کارت `assets/vocab/numerale/` (dreihundertste، dreißigste، dreiundzwanzigste، dreizehnte، dritte؛ `wortartKorrektur`)؛ ۹ کلمه‌ی نامطمئن در `zurueckgestellt` (PLAN «🏁 مرحله‌ی آخر» #109–117). ایندکس‌ها با `vokab_index.dart` تازه شدند. هیچ فایل کد/ظاهر عوض نشد.
- **2026-09-26 (دور ۱۴۴–۱۶۳):** ۲۰۰ کارت در `assets/vocab/adjektiv/` (buchhalterisch … diskretionär) + ۱ کارت `assets/vocab/adverb/adverb_demgemaess.json` (Wortart درست: قید، `wortartKorrektur`)؛ ۲۹ کلمه‌ی نامطمئن در `zurueckgestellt` (buchen … diptotisch، فهرست: PLAN «🏁 مرحله‌ی آخر» #80–108). `assets/vocab_index.json` و `assets/vocab_formen/` با `vokab_index.dart` تازه شدند. هیچ فایل کد/ظاهر عوض نشد.
- **2026-09-26 (دور ۱۳۹–۱۴۳):** ۵۰ کارت صفت در `assets/vocab/adjektiv/` (böse … brutal)؛ ۶ کلمه‌ی نامطمئن در `zurueckgestellt` (bonfortionös، botrytisiert، boustrophedon، brennheiß، brillanten، brokaten). هیچ فایل کد/ظاهر عوض نشد.
- **2026-09-26 (دور ۱۳۰–۱۳۸):** ۸۸ کارت صفت + billiardste/billionste در `assets/vocab/numerale/` (`wortartKorrektur`)؛ ۱۰ کلمه‌ی نامطمئن در `zurueckgestellt`. هیچ فایل کد/ظاهر عوض نشد.
- **2026-09-26 (دور ۱۰۰–۱۲۹):** ۳۰۰ کارت صفت در `assets/vocab/adjektiv/` + baldmöglichst در `assets/vocab/adverb/` (`wortartKorrektur`)؛ ۴۰ کلمه‌ی نامطمئن در `zurueckgestellt` (`tool/naechste_woerter.dart`) و «🏁 مرحله‌ی آخر» در PLAN. هیچ فایل کد/ظاهر عوض نشد.
...
| `web/telegram_guard.js` | **L.3c ✅** (2026-09-24 دور ۶۴) — läuft in `web/index.html` **nach** `locale_guard.js`, **vor** `flutter_bootstrap.js`. Nur wenn die Adresse `tgWebAppData` enthält (Start als Telegram-Mini-App): per `document.write` synchron das offizielle SDK `telegram.org/js/telegram-web-app.js` laden (es liest `location.hash` selbst ⇒ `Telegram.WebApp.initData`), danach Adresse per `history.replaceState` säubern (`#/pfad?tgWebAppData=…` ⇒ `#/pfad`, sonst ohne `#`). Sonst go_router: „GoException: no routes for location". Normale Starts: kein SDK, keine Änderung. ⚠️ Nutzerdaten nur aus `Telegram.WebApp.initData`, nie aus der Adresse — Dart liest heute gar keine Telegram-Daten (`Uri.base` nur für Supabase-Mail-Links in `main.dart`) |
...
| `tool/pwa/offline_test.cjs` | **L.3 ✅** — Chrome واقعی (`CHROME_PATH`): آنلاین ⇒ worker فعال ⇒ Offline + goto ⇒ همه‌ی فایل‌های بسته با fetch آفلاین ⇒ پنجره‌ی تازه آفلاین. ملاک «VOX بالا آمد» = `#vox-loading` حذف شده. ⚠️ `page.reload()` در Playwright با Offline از SW رد می‌شود ⇒ `goto` · **مرحله‌ی ۵ (L.3c):** هر آدرس در پنجره‌ی تازه، آفلاین: `#tgWebAppData=…` ⇒ بدون tgWebAppData، `#/wortschatz?tgWebAppData=…` ⇒ `#/wortschatz`، `#/` و `#/wortschatz` مثل قبل؛ هر پیام GoException ⇒ قرمز |
...
| `core/backup/pruefungs_ergebnis.dart` | **T.1 (2026-09-25)** — `PruefungsErgebnis`/`PruefungsTeil`، `neuePruefungsId` (`pr:#`+32 hex)، `listeLesen` (بخشنده)، `listeSchreiben` (مرتب)، `vereinigen` (فقط افزودن)، **دور ۸۷:** `zeitLimitSekunden`/`zeitAbgelaufen`، `teilReihenfolge`/`teileGeordnet` (ترتیب کارنامه)، `vollstaendig()` (فقط آزمون کامل). Reines Dart. قرارداد **Fassung 5**؛ ذخیره: `kPruefungenKey`=`vox_pruefungen_v1` + `pruefungenLesen`/`pruefungMerken` در `user_state_repository.dart`؛ Provider: `features/pruefungen/controllers/pruefungs_ergebnisse_controller.dart`. ⚠️ پاسخ‌ها هرگز ذخیره نمی‌شوند. |
