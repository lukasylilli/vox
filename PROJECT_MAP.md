...
| `core/utils/anmelde_adresse.dart` (+`_io`/`_web`) | **2026-09-22** — `entferneAnmeldeParameter()`: nimmt `?token_hash=…&type=recovery` nach dem Einlösen aus der Adresszeile (`history.replaceState`, `#/…` bleibt). Weiche wie `dokument_sprache.dart` |
| `core/services/auth_service.dart` | **S.3 ✅** — einzige Stelle, die `supabase_flutter` kennt. ⚠️ **Kein Benutzername** (`profiles` gehört Root-in). **`deleteAccount()` ✅ L.1d (2026-09-22)** · **`verifyRecoveryToken()` + `wiederherstellungsToken(Uri)` (2026-09-22): Passwort-Link über `token_hash`, unabhängig vom Browser-Speicher (PKCE-Verifier fehlte in anderem Browser)** → `AccountDeletion {deleted, unavailable, failed}`, zeichengleich zu Root-in — löscht `auth.users` = Konto in **beiden** Apps (Entscheidung Lukas). Gibt `AuthResult` zurück statt zu werfen; `AuthIssue` wird in der Oberfläche übersetzt, nicht hier |
