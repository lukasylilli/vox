// FILE: lib/features/vokabular/widgets/wort_karte_inhalt.dart
// PHASE: L.4b (2026-09-24) — aus wort_seite_screen.dart herausgelöst.
// PURPOSE: Der Inhalt EINER Prompt-Karte (Schema 3.0) als Bausteine — EINE
//          Quelle für beide Seiten, die eine Karte zeigen:
//            · wort_seite_screen.dart  (Karte ohne altes App-Wort)
//            · word_detail_screen.dart (altes App-Wort: seine Seite wird um die
//              Karte ERWEITERT, nichts davon gelöscht — Regel L.4b, Lukas)
//          [WortKarteKopf]       Symbol + Wort + IPA + Niveau → Übersetzung → Hinweis
//          [WortKarteAbschnitte] Beispiele → Kasus im Beispiel → Details je
//                                Wortart → Synonyme/Gegenteil/Komposita →
//                                Wortbildung → Wortnetz (klickbar) → Meine Notiz
//          Aussehen (Farbe/Form) nur aus core/grammatikon — nie aus der Karte.
...
import 'kasus_beispiel_block.dart';
import 'wort_notiz.dart';
import 'wortseite_bausteine.dart';

/// Kopf einer Karte: Symbol + Wort (Artikel in Genusfarbe) + IPA + Niveau,
/// darunter Übersetzung (aktive Sprache) und Hinweis (`anmerkung`, Deutsch).
class WortKarteKopf extends StatelessWidget {
  final Map<String, dynamic> card;
  const WortKarteKopf({super.key, required this.card});
  ...
  Widget build(BuildContext context) {
    ...
    final farbe = descriptorColor == GrammatikonSpec.kontur
        ? null
        : descriptorColor;
...
/// Alle Abschnitte einer Karte unter Kopf und Aktionen.
class WortKarteAbschnitte extends StatelessWidget {
  final Map<String, dynamic> card;

  /// id → Index-Eintrag (NICHT volle Karten) — nur für „gibt es das Wort?".
  final Map<String, Map<String, dynamic>> byId;
  const WortKarteAbschnitte({
    super.key,
    required this.card,
    required this.byId,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final etymologie = etyRoh is Map
        ? vokabUeb(context, etyRoh)
        : etyRoh as String?;
    ...
    return Column(
      ...
      children: [
        ...
        if (etymologie != null && etymologie.isNotEmpty)
          Sektion(
            titel: 'Wortbildung',
            children: [BeispielBlock(satz: etymologie)],
          ),
  ...
  Widget _beispiele(BuildContext context) {
    ...
    return Sektion(
      titel: AppL10n.t(context, 'examples'),
      children: [
  ...
  Widget _kasusBeispiele(BuildContext context) {
    ...
    final hatDativ = list.any(
      (b) => ((b['dativ'] as List?) ?? const []).isNotEmpty,
    );
    final hatAkk = list.any(
      (b) => ((b['akkusativ'] as List?) ?? const []).isNotEmpty,
    );
    return Sektion(
      titel: 'Kasus im Beispiel',
      children: [
    ...
    return Sektion(
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(
            AppL10n.t(context, 'wortnetz_hint'),
