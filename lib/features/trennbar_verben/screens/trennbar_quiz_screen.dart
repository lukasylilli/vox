// FILE: lib/features/trennbar_verben/screens/trennbar_quiz_screen.dart
import 'dart:math';
...
class TrennbarQuizScreen extends StatefulWidget {
  const TrennbarQuizScreen({super.key, required this.verbs});

  final List<TrennbarVerb> verbs;

  @override
  State<TrennbarQuizScreen> createState() => _TrennbarQuizScreenState();
}

class _TrennbarQuizScreenState extends State<TrennbarQuizScreen> {
  final _rng       = Random();
  ...
  _Question _makeQuestion(_QuizType type, TrennbarVerb verb) {
    switch (type) {
      ...
      case _QuizType.cloze:
        ...
        final stem  = verb.prefix;
        final ti    = words.indexWhere(
            (w) => w.toLowerCase().contains(stem.toLowerCase()));
        if (ti == -1) {
          ...
          children: [
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    PrefixTypeBadge(q.verb.prefixType),
