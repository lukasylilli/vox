...
>
> 🗓️ **آخرین جلسه:** 2026-09-20 (دور ۳) — Lukas: سؤال درباره‌ی **برنامه‌ی هفتگی در Root-in** (عادت فقط سه‌شنبه‌ها / سه‌شنبه و پنجشنبه / سه بار در هفته). **VOX دست‌نخورده** — این ورودی فقط یادداشت است، هیچ فایل کدی در این ریپو عوض نشد. **Root-in:** «Phase 32 — Wochenplan» ساخته و روی `main` منتشر شد (schema 4→5، فرمت پشتیبان Root-in نسخه ۱→۲؛ analyze تمیز، ۲۷۹ تست سبز، Deploy سبز). برای VOX هیچ تغییری لازم نیست: VOX فقط در `vox_backups` می‌نویسد (نه `backups`/`profiles` از Root-in) و لینک «Routine» در `app_links.dart` همان است. جزئیات: `Root-in/PLAN.md` → Phase 32.
>
...
> ⏳ **در حال انجام (ثبت «قبل از کار»):** کامپوننت مشترک `core/widgets/wort_popup.dart` + `klick_wort_text.dart` + قاعده‌ی پیدا کردن کلمه `core/wort/klick_wort.dart`؛ اتصال به Lesen، گرامر، Redemittel. نتیجه‌ی نهایی و CI در ورودی بعدی همین‌جا ثبت می‌شود.
>
