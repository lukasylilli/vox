...
- [x] **تست RTL همه‌ی صفحات — باگ پیدا و رفع شد** ✅ (گزارش Lukas + رفع Claude، 2026-09-18)
      ...
      **رفع (ماندگار، نه موضعی):** ویجت مشترک `lib/core/widgets/deutsch_text.dart` → `DeutschText`، که
      ...
      ⚠️ **باقی‌مانده‌ی این باگ:** جاروی بقیه‌ی صفحه‌هایی که آلمانی نشان می‌دهند (صفحه‌های grammatik،
      wortschatz، lesen) هنوز انجام نشده — `uebung_karte.dart` از قبل درست بود، بقیه باید یکی‌یکی چک شوند.
