...
>
> 🗓️ **آخرین جلسه:** 2026-09-23 (دور ۲۰) — پاپ‌آپ کلمه، گزارش دوم Lukas (آیفون): (۱) در German News باز نمی‌شد ⇒ توضیح خبر حالا `KlickWortText` است (`news_screen.dart`). (۲) پاپ‌آپ دکمه‌ی تلفظ نداشت ⇒ `_WortKopf` در `wort_popup.dart`: کلمه + تلفظ برای **هر** کلمه، حتی بدون کارت. (۳) روی آیفون **هر** کلمه «در دیکشنری نیست» (حتی aalartig) — در Chromium و WebKit (شبیه‌سازی iPhone، fa و en) پیدا می‌شود ⇒ بازتولید نشد. رفع ریشه‌ای بدون حدس: قبلاً اگر فهرست کلمه‌ها (vocab_index) بارگذاری نمی‌شد، خطا بی‌صدا «در دیکشنری نیست» نشان داده می‌شد؛ حالا پیام جدا «فهرست کلمه‌ها بارگذاری نشد» + دکمه‌ی «تلاش مجدد» (`klick_wort_provider.dart`). آزمایش بعدی Lukas نشان می‌دهد کدام است. analyze سبز، ۲۹۵ تست سبز (+۲).
...
> - [ ] بازبینی چشمی: ✅ صفحه‌ی پروفایل (فاز P) · پاپ‌آپ کلمه (L.5f): باز می‌شود؛ News + تلفظ رفع شد (دور ۲۰)؛ **«همه‌چیز: در دیکشنری نیست» روی آیفون** — منتظر آزمایش دوباره‌ی Lukas (اگر حالا «بارگذاری نشد» بیاید ⇒ مشکل بارگذاری فهرست است).
> - [ ] **(Claude، بعد از آن)** جمله‌های آلمانی کوتاه در رابط فارسی راست‌چین‌اند + `anmerkung` صفحه‌ی کلمه بدون `DeutschText` (یافته‌ی دور ۲۰).
>
