...
> **Lukas 2026-09-16:** «منابع همه‌چیز با من است، برنامه‌نویسی با تو.» ⇒ برای هر مورد باز پایین، وقتی نوبتش رسید **منبع را از Lukas بخواه** (هر قالبی: Excel، CSV، JSON، Word، PDF) — نه حدس، نه تولید از صفر.
...
- [!] **A.6 ⛔ روش ساخت کلمه‌ها — تصمیم با Lukas (ثبت 2026-09-16)**
      **⚠️ قاعده برای Claude:** این قدم را **هرگز خودسرانه شروع نکن** — حتی اگر اولین قدم باز PLAN باشد
      ...
      | **۰ — تبدیل منابع خود Lukas** (پیشنهاد 2026-09-16) | رایگان | هزاران کارت در چند دقیقه | منبع کلمه با ترجمه‌ی fa/en + مسیر «فقط import». طبق `vokab_schema.dart` فقط `wort`، `wortart`، `uebersetzung` (fa+en)، `niveau` و شیء `details` حیاتی‌اند؛ بقیه فقط Warnung ⇒ کارت از جزوه معتبر است و فیلدهای ناقص **خالی می‌مانند** (قاعده‌ی «نماد نباشد بهتر از نماد غلط»)، بعداً فقط همان‌ها تکمیل می‌شوند ⇒ هزینه‌ی AI خیلی کمتر |
      ⚠️ برای روش ۰ اول باید یک نمونه از منبع کلمه‌ی Lukas دیده شود (کدام ستون‌ها موجود است).
