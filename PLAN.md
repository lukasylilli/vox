...
> ⚠️ **وضع کد (2026-09-24):** حالت «کلمه‌ی تنها» (`helfen` ↔ `verb_helfen`) ✅ کار می‌کند. حالت «کلمه + حرف اضافه» (`denken an`، `abhängig von`) هنوز جفت نمی‌شود ⇒ L.4b-2 (بخش L.4) — ⛔ یک سؤال از Lukas باز است.
...
- [ ] **L.4b-2 کلمه + حرف اضافه‌ی ثابت (ورودی‌های قبلی deck Präpositionen)** — طبق قاعده‌ی «الف» این‌ها کلمه‌اند، پس صفحه‌ی قبلی‌شان باید با کارت همان lemma گسترش پیدا کند (نه صفحه‌ی جدید).
      **وضع امروز (شمارش 2026-09-24):** ۸ lemma هم ورودی قبلی «+ حرف اضافه» دارند هم کارت: abhängig (von)، antworten (auf)، erzählen (von)، gehören (zu)، glauben (an)، stolz (auf)، vertrauen (auf، in)، warten (auf). برای abhängig، stolz، warten الان **دو ردیف و دو صفحه** در «Alle Wörter» هست (ردیف قبلی + کارت جدید) — خلاف قاعده، باید درست شود.
      ⛔ **سؤال باز از Lukas (قبل از کد):** بعضی کلمه‌ها چند صفحه‌ی قبلی دارند (مثلاً `vertrauen` + `vertrauen auf` + `vertrauen in`؛ `arbeiten an` + `arbeiten bei` — در کل ۴۱ lemma با بیش از یک ورودی). پیشنهاد Claude: کارت (یک فایل) **زیر همه‌ی صفحه‌های قبلی همان lemma** نشان داده شود (اصل «یک محتوا، چند ورودی»)؛ ردیف کارت در لیست حذف؛ لینک کارت ⇒ صفحه‌ی قبلی بدون حرف اضافه اگر هست، اگر فقط یکی هست همان، اگر چندتاست ⇒ ؟ (انتخاب Lukas). تا جواب Lukas کدی ساخته نمی‌شود.
