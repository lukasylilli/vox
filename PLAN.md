...
> ⏭️ **قدم بعدی:** (۱) `VoxOptionButton` جهت‌پذیر شود تا گزینه‌های آلمانیِ کوییز هم درست شوند (قدم مستقل،
> چون دکمه‌ی مشترک نگهبان CI دارد) · (۲) باگ آفلاین — منتظر DevTools از Lukas · (۳) درس بعدی کتاب / منبع
> deck بعدی از Lukas.
>
...
- [x] **تست RTL همه‌ی صفحات — باگ پیدا و رفع شد** ✅ (گزارش Lukas + رفع Claude، 2026-09-18)
      ...
      **درسِ روش کار:** «این فایل را درست کردم» کافی نیست — باید در هر فایل **همه‌ی** `Text(`ها یکی‌یکی
      ...
      ⚠️ **یک مورد باقی‌ماند، عمداً:** گزینه‌های پاسخ در `redemittel_1010_quiz_screen.dart` از
      `VoxOptionButton(label: String)` می‌گذرند. برای درست‌شدن باید خودِ آن دکمه‌ی مشترک یک پارامتر جهت
      بگیرد — و آن دکمه با `test/puzzling_buttons_test.dart` نگهبانی می‌شود. این تغییر ریسک بیشتری دارد و
      باید قدم مستقل خودش باشد، نه ضمیمه‌ی این جارو.
- [ ] **تست آفلاین — باگ تأیید شد، علت هنوز قطعی نیست** (گزارش Lukas، 2026-09-18)
      ...
      **آنچه بررسی شد:** حجم کل `assets/` فقط **۷٫۵ مگابایت** است (data 4.3 · images 2.6 · vocab 0.36 ·
      ...
      نیست**؛ precache کاملاً شدنی است. کلمه‌ها هم از `rootBundle` می‌آیند (`vokabular_controller.dart`)،
...
- [ ] **L.5f رفتار یکسان کلیک روی کلمه، در همه‌جای اپ.** هر جای اپ که کلمه‌ای نمایش داده می‌شود، کلیک روی
