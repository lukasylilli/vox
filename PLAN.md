...
>
> 🗓️ **آخرین جلسه:** 2026-09-24 (دور ۴۹–۵۳، یک چت) — **۵۰ کلمه‌ی تازه** در ۵ دور: agogisch … allein ⇒ آرشیو **۲۸۶ کارت** (هر دور ۰ خطا/۰ هشدار؛ ۳۰۹ تست سبز). **کنار گذاشته شدند:** aldente (املا مشکوک؛ Duden: «al dente») و aleppinisch (کاربرد صفتی نامطمئن) ⇒ «🏁 مرحله‌ی آخر». **ادامه‌ی کلمه‌ها از: alleine.**
...
>
> 📜 **کارنامه (خیلی خلاصه):**
> - **2026-09-24** — ۵۰ کلمه‌ی تازه (دور ۴۹–۵۳، agogisch … allein) · ۲۹ کلمه (دور ۴۵–۴۷) · ۱۰۰ کلمه‌ی تازه (دور ۳۴–۴۳، abenteuerlich … adsorptiv) · L.3 آفلاین: `vox_sw.js` (Workbox، `tool/pwa/`) + تست آفلاین Chrome در deploy · ده کلمه‌ی جدید (دور ۳۲) · تصمیم اسم‌ها (دور ۳۱).
...
1. [~] **L.3 تست آفلاین روی کامپیوتر — Claude ✅ رفع شد (2026-09-24 دور ۳۳)؛ ⏭️ Lukas دوباره روی کامپیوتر تست کند** (Service Workers باید `vox_sw.js` سبز نشان دهد، Cache Storage باید `vox-precache-…` داشته باشد، بعد Offline + Reload). جزئیات: بخش L.3.
2. [~] **پاپ‌آپ کلمه برای شکل‌های صرف‌شده — Claude ✅ ساخته شد (دور ۲۴)**؛ ⏭️ Lukas روی آیفون امتحان کند (مثلاً «aalartigen» در مثال صفحه‌ی aalartig).
3. [~] **RTL در Auswendiglernen — Claude ✅ کامل (دور ۲۶–۲۷)؛ ⏭️ Lukas روی آیفون نگاه کند.** (متن‌های توضیحی گرامر که فارسی و آلمانی را در یک جمله قاطی دارند عمداً دست نخوردند — جدا کردنشان یعنی تغییر داده‌ها.) سابقه: سطرهای آلمانی که نام فیلدشان برای نگهبان ناشناس است: «sich auskennen mit + dativ» (Reflexiv)، مترادف NVV («sich verabschieden»)، چیپ «von +Dat»، عنوان کانکتورها در فهرست، و جمله‌های آلمانی در صفحه‌های گرامرِ deckها (`*_grammar_screen.dart`). بعدش Lukas یک نگاه روی آیفون.
4. [ ] **L.6 درس بعدی کتاب گرامر — Lukas منبع بفرستد (اختیاری؛ می‌تواند بعد از انتشار بیاید).** رسیده: درس ۱ و ۲.
...
>
> 🔒 **قاعده‌ی ثابت: کلمه‌ای که از قبل در اپ هست (Lukas، 2026-09-24) — L.4b:**
> - **چه کلمه‌ای؟** کلمه‌ای که از قبل در اپ بود — کلمه‌های همراه اپ در دیتابیس قدیمی (`Words` با `ausApp = true`؛ از deckهای Dativ/Akkusativ، Konnektoren، NVV، Präpositionen؛ صفحه‌اش `features/wortschatz/screens/word_detail_screen.dart`).
> - **چه اتفاقی می‌افتد؟** خود کلمه و **هیچ چیز از صفحه‌اش حذف نمی‌شود** (معنی، مثال، صرف، یادداشت گرامری، لایتنر، دسته‌بندی — همه می‌مانند). **صفحه‌ی قبلی گسترش پیدا می‌کند:** کارت کامل طبق پرامپت کلمه ساخته می‌شود (مثال‌ها و ترجمه‌ی قبلی ورودی پرامپت‌اند) و همه‌ی بخش‌هایش (نماد Grammatikon، IPA، ترجمه، مثال‌ها، Details، Synonyme/Gegenteil/Komposita، Wortbildung، Wortnetz، یادداشت) به **همان صفحه‌ی قبلی** اضافه می‌شود.
> - **یک کلمه = یک صفحه:** صفحه‌ی قبلی تنها صفحه‌ی کلمه است؛ لیست «Alle Wörter» فقط یک ردیف (ردیف قبلی) نشان می‌دهد؛ هر لینک به کارت (Wortnetz، پاپ‌آپ، deckها) به همان صفحه‌ی قبلی می‌رود.
> - **جفت کردن کلمه‌ی قبلی با کارت:** فقط وقتی lemma دقیقاً یکی است و Wortart سازگار است و جفت یکتاست — وگرنه جفت نمی‌شود (قاعده‌ی ۳: حدس ممنوع). تنها منبع: `features/wortschatz/data/altwort_karte.dart`.
> - در روال ده کلمه: این کار **خودکار** است — کارت با id معمول (`vokabId`) ساخته می‌شود و کد خودش آن را به صفحه‌ی قبلی وصل می‌کند؛ هیچ فایل دیگری عوض نمی‌شود.
...
4. کلمه‌ای که مطمئن نیستی ⇒ **نساز** و طبق «🔒 قاعده‌ی ثابت: کلمه‌هایی که وارد اپ نمی‌شوند» (بالا) ثبتش کن؛ کلمه‌ی بعدی جایش. بعد برای همان ۱۰ کلمه، طبق پرامپت یک **JSON-Array** بساز (همه‌ی قاعده‌های TEIL 0 — به‌ویژه ۱۴: «مطمئن نیستی ⇒ null، حدس نزن»؛ ۱۵: حرف اضافه‌ی ثابت اجباری). در `/tmp/batch.json`.
...
- کارت‌های دیگر آرشیو (افعال Dativ و …) از deckها آمده‌اند، نه از این فهرست‌ها. کل آرشیو: **۲۸۶ کارت** (2026-09-24 دور ۵۳).
...
- [~] **تست آفلاین — علت پیدا و رفع شد ✅ (2026-09-24 دور ۳۳)؛ ⏭️ تأیید Lukas روی کامپیوتر**
...
- [~] **L.4b گسترش صفحه‌ی کلمه‌های قبلی (Lukas، 2026-09-24)** — قاعده: «📚 روال ده کلمه جدید» → «🔒 کلمه‌ای که از قبل در اپ هست». شروع: ۷۵ کلمه‌ی قبلی که کارت پرامپتی‌شان از قبل هست (۷۴ فعل Dativ/Akkusativ + weil). ⏳ در حال انجام.
      ⚠️ اصلاح عدد: در گفت‌وگو «۶۶» گفته شد (شمارش تقریبی)؛ شمارش دقیق با lemma + Wortart = **۷۵**. der (Konnektor ≠ Artikel) و doch (Konnektor ≠ Partikel) جفت نمی‌شوند.
